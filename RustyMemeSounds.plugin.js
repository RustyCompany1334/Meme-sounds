 ...
 * @authorId 788728528953802773
 * @authorLink https://github.com/RustyCompany1334/
 * @source https://github.com/RustyCompany1334/Meme-sounds/blob/main/RustyMemeSounds.plugin.js
 * @updateUrl https://raw.githubusercontent.com/RustyCompany1334/Meme-sounds/blob/main/RustyMemeSounds.plugin.js
 */

module.exports = (() => {
	
	/* Configuration */
	const config = {info: {name: "Meme Sounds (edited by RustyCompany1334)", authors: [{name: "RustyCompany1334#6087", discord_id: "788728528953802773", github_username: "Lonk12", twitter_username: "wolfyypaw"},{name: "FlyMaster#2642", discord_id: "459726660359553025", github_username: "Apceniy"}], version: "0.6.1", description: "J'ai pris le plugin meme sounds et je l'ai modifie et ajoute plus de sons (bon eu je peux pas mettre d'acents)", github: "https://github.com/RustyCompany1334/Meme-sounds/blob/main/RustyMemeSounds.plugin.js", github_raw: "https://raw.githubusercontent.com/RustyCompany1334/Meme-sounds/blob/main/RustyMemeSounds.plugin.js"}, defaultConfig: [{id: "setting", name: "Sound Settings", type: "category", collapsible: true, shown: true, settings: [{id: "LimitChan", name: "Limit to the current channel only.", note: "When enabled, sound effects will only play within the currently selected channel.", type: "switch", value: true}, {id: "delay", name: "Sound effect delay.", note: "The delay in miliseconds between each sound effect.", type: "slider", value: 200, min: 10, max: 1000, renderValue: v => Math.round(v) + "ms"}, {id: "volume", name: "Sound effect volume.", note: "How loud the sound effects will be.", type: "slider", value: 1, min: 0.01, max: 1, renderValue: v => Math.round(v*100) + "%"}]}], changelog: [{title: "New Stuff", items: ["Added the vine boom sound effect when :moyai: is sent into chat.", "Thanks to Orangenal name#9280 for adding vine boom!"]}]};
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				{re: /nice/gmi, file: "noice.mp3", duration: 600},
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/RustyCompany1334/Meme-sounds/blob/main/"+sound[1].file);
